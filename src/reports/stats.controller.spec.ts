...
describe('StatsController', () => {
  ...
  const mockCsv = 'Date,Category,Vendor,Amount,Currency,Status\n2026-04-15,Food,Amazon,50.00,USD,APPROVED';
  ...
  describe('exportCsv', () => {
    it('should return a StreamableFile with CSV data', async () => {
      ...
      const result = await controller.exportCsv(userId, { month: 4, year: 2026 } as any, mockRes);

      expect(service.exportCsv).toHaveBeenCalledWith(userId, { month: 4, year: 2026 });
    ...
    it('should use current month/year when no query params', async () => {
      ...
      await controller.exportCsv(userId, {} as any, mockRes);
      ...
      expect(mockRes.set).toHaveBeenCalledWith({
        'Content-Type': 'text/csv',
        'Content-Disposition': expect.stringContaining(`expenses-${year}-${String(month).padStart(2, '0')}.csv`),
      });
    });
  });
});
